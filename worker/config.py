...
def load() -> Config:
    ...
    return Config(
        ...
        # .strip() throughout: pasted values arrive with stray whitespace,
        # and a newline inside a Graph URL breaks the request outright.
        meta_app_id=os.environ.get("META_APP_ID", "").strip() or None,
        meta_app_secret=os.environ.get("META_APP_SECRET", "").strip() or None,
        meta_system_user_token=os.environ.get("META_SYSTEM_USER_TOKEN", "").strip() or None,
        meta_ad_account_id=os.environ.get("META_AD_ACCOUNT_ID", "").strip() or None,
        meta_page_id=os.environ.get("META_PAGE_ID", "").strip() or None,
        meta_instagram_actor_id=os.environ.get("META_INSTAGRAM_ACTOR_ID", "").strip() or None,
        meta_api_version=os.environ.get("META_API_VERSION", "v26.0").strip() or "v26.0",
    )
